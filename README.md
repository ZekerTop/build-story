...
沟通复盘只在同一会话中找到 `user → assistant → user correction` 顺序证据时生成，最多显示 3 张卡。重复提示本身不等于表达不清；BuildStory 会区分 **AI 忽略了明确要求**、**双方对词义理解不同**与 **需求在看到结果后演化**，不会默认把责任归给用户。仍然证据不足的候选默认不生成卡，也可以由用户把已有候选确认为“证据不足”。只有证据支持用户侧改写时，才会给出“下次可以这样说”。
...
沟通复盘只分析用户明确授权的会话。它要求同一会话中出现连续的 `user → assistant → user correction` 证据链，并默认最多保留 3 个案例。
