...
BuildStory 是一个本地优先的 Agent Skill 和项目复盘报告生成器。它从 Git 历史和用户明确授权的 AI 编程会话中，提炼项目故事与关键转折，发现可能的重复返工、回滚和注意力黑洞。有已授权会话时，它还会复盘哪些信息是在 AI 行动后才变清楚的，并在证据支持时给出可直接复用的改写示例，再将真实证据转化为经验、作品集、简历和面试材料。
...
9. 基于 `user → assistant → user correction` 证据链的沟通复盘，最多展示 3 张卡；
10. 可用于作品集、简历、成就记录和 STAR 面试故事的证据卡片。

活动脉搏描述可观察到的 Git 和会话证据，不把提交次数解释为勤奋程度或生产力。

完整提交记录和数字计算方法仍然保留，但默认折叠为支撑证据。BuildStory **不会**给项目或 Prompt 计算一个虚假的总分，也不评价用户的表达能力。

当前生成器版本为 **0.4.0**，结构化证据 schema 为 **1.5**。
...
当前解析器支持通用的 `.jsonl`、`.json`、`.txt` 和 `.md` 输入，也会保守识别常见的嵌套 `message`、`payload` 与会话归档结构。同一文件出现多个 session 时，无法安全归属的消息段会彼此隔离，宁可少生成卡片，也不跨会话拼接。不同 Agent 的字段命名仍可能不同；无法识别时间或角色时，会保留 Git 报告并降低相关结论的置信度，不会把缺失内容编造成事实。

大型 Codex `.jsonl` 会话不会再因为超过通用文件大小限制而被整份跳过；当前可读取至 128 MB，并继续受全局事件数量上限保护。非流式 `.json`、`.txt` 和 `.md` 仍保留更严格的 20 MB 限制。

沟通复盘只在同一会话中找到 `user → assistant → user correction` 顺序证据时生成，最多显示 3 张彼此独立的卡。分析器会回看“可以，开始吧”之前的真实要求，也会把“另外还有一个 Bug”从当前纠正链中拆开，而不是把多个问题混成一张卡。重复提示本身不等于表达不清；BuildStory 会区分 **AI 忽略了明确要求**、**双方对词义理解不同**与 **需求在看到结果后演化**，不会默认把责任归给用户。仍然证据不足的候选默认不生成卡，也可以由用户把已有候选确认为“证据不足”。只有证据支持用户侧改写时，才会给出改写；改写必须重新组织目标、范围、边界和验收方式，不能只是给用户后来原话套一层模板。

对话与 Git 提交的时间接近只是可检查线索，**不代表两者存在因果关系**。报告只保留解释结论所需的短摘录，不上传、不复制完整会话。

作为 Agent Skill 使用时，可以直接说：

```text
使用 $build-story 复盘当前项目，并检查已授权会话中哪些信息总是在 AI 行动后才补充。只给我最有证据的 3 个案例；仅在证据支持时给出下次可直接使用的说法，不要给 Prompt 打分。
```
  ...
  "zh": {
    ...
    "communication_confirmations": {
      "communication:示例ID": {
        ...
        "reason": "原始要求已明确说明保留中英文切换，这次更像 AI 执行时遗漏。",
        "analysis": "用户无需把原要求写得更长；AI 应在执行前复述必须保留的边界。",
        "lesson": "当边界已经写清仍被遗漏时，应优先检查 AI 的执行，而不是责怪用户表达。"
      }
    }
  }
}
...
```text
使用 $build-story 检查已授权会话中的沟通纠正链，区分 AI 忽略、词义不一致、需求演化和证据不足；最多给我 3 个案例，只在证据支持时给出可直接复用的改写。
```
...
沟通复盘只分析用户明确授权的会话。它要求同一会话中出现 `user → assistant → user correction` 证据链，并默认最多保留 3 个彼此独立的案例。短确认会回溯到前一条真实要求，混入同一条消息的另一个问题会先拆开。

重复提示不自动等于表达不清。结论可以是信息后来才补全、AI 忽略了明确要求、双方对同一个词理解不同、需求在看到结果后演化，或证据不足无法归因。这些标签评估人与 AI 如何对齐，不评价用户的表达能力。
...
- 报告中不会包含本机绝对路径；
- 不会将完整会话复制进报告；
- 沟通复盘只保留支撑结论所需的短摘录，不上传会话；
- 自动排除环境上下文、AGENTS 指令等注入事件，并隐藏本机用户目录与临时路径；
- 简历和作品集不得编造影响指标。
...
- 提交信息分类使用启发式规则；
- 高频变更并不自动等于浪费；
- 不同 AI 工具的会话格式差异很大，v0.4.0 仍使用保守的通用解析方式；
- 沟通纠正链只能展示对齐过程，无法单独证明责任属于用户或 AI；
- 对话与 Git 提交的时间接近不代表因果关系；
