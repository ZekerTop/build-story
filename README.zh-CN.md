...
BuildStory 是一个本地优先的 Agent Skill 和项目复盘报告生成器。它可以从 Git 历史和用户明确授权的 AI 编程会话中，重建项目时间线，发现可能的重复返工、回滚和注意力黑洞，生成基于证据的过程画像，并将真实证据转化为复盘、作品集、简历和面试材料。
...
2. 明确回滚、高频变更和重复循环候选；
3. 注意力区域和活跃时间估算；
4. 交付、验证、可追溯性、迭代控制和经验沉淀的分维度画像；
5. 可用于作品集、简历、成就记录和 STAR 面试故事的证据卡片。

BuildStory **不会**给项目计算一个虚假的总分。
...
将完整的 `build-story` 文件夹克隆或复制到客户端的用户级或项目级 Skills 目录。请保留 `SKILL.md`、`scripts/`、`references/`、`assets/` 和 `agents/`。
...
这些分数描述仓库中可观察到的证据，不衡量一个人的智力、职级或价值。
